...
public class PublicChatRoomsActivity extends BaseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(com.kind.chat.R.layout.activity_public_groups);

		// 搜索框
		searching = (EditText)findViewById(com.kind.chat.R.id.query);
		clean = (ImageButton)findViewById(com.kind.chat.R.id.search_clear);
		searching.setHint(com.kind.chat.R.string.search);
		inputMethodManager = (InputMethodManager)getSystemService(Context.INPUT_METHOD_SERVICE);
		
		pb = (ProgressBar) findViewById(com.kind.chat.R.id.progressBar);
		listView = (ListView) findViewById(com.kind.chat.R.id.list);
		title = (TextView)findViewById(com.kind.chat.R.id.tv_title);
		title.setText(getResources().getString(com.kind.chat.R.string.chat_room));
		chatRoomList = new ArrayList<EMChatRoom>();
		rooms = new ArrayList<EMChatRoom>();
		
		View footView = getLayoutInflater().inflate(com.kind.chat.R.layout.listview_footer_view, null);
        footLoadingLayout = (LinearLayout) footView.findViewById(com.kind.chat.R.id.loading_layout);
        footLoadingPB = (ProgressBar)footView.findViewById(com.kind.chat.R.id.loading_bar);
        footLoadingText = (TextView) footView.findViewById(com.kind.chat.R.id.loading_text);
...
//                                    footLoadingLayout.setVisibility(View.VISIBLE);
                            ...
                            if(isFirstLoading){
                                ...
                                if(chatRooms.size() < pagesize){
                                    ...
                                    footLoadingText.setText(getResources().getString(com.kind.chat.R.string.no_more_messages));
                                }
                                adapter.notifyDataSetChanged();
                            }
                            isLoading = false;
                    ...
                    runOnUiThread(new Runnable() {
                        public void run() {
                            ...
                            pb.setVisibility(View.INVISIBLE);
                            footLoadingLayout.setVisibility(View.GONE);
                            Toast.makeText(PublicChatRoomsActivity.this, getResources().getString(com.kind.chat.R.string.failed_to_load_data), 0).show();
                        }
                    });
                }
            }
	...
	private class ChatRoomAdapter extends ArrayAdapter<EMChatRoom> {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			((TextView) convertView.findViewById(com.kind.chat.R.id.name)).setText(getItem(position).getName());
